...
export default function StuckInArticleClassifier() {
  ...
  const handleBulkDownload = () => {
    ...
    const filename = generateFilename('stuck-article-classifier', data.total);
    downloadCSV(data.results as CSVData[], filename);
  };

  const handleSingleDownload = (item: StuckDataItem) => {
    downloadSingleRow(item as CSVData, 'stuck-article-classifier');
  };
            ...
            <CardContent className="p-6">
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">No documents are currently stuck in article classification.</p>
                </div>
              ) : (
                <div>
                  <div className="mb-6 p-4 bg-gradient-to-r from-orange-50 to-red-50 rounded-lg border-l-4 border-orange-400">
                    <h3 className="font-semibold text-orange-800 mb-1">
                      ⚠️ {data.total} documents require attention
                    </h3>
